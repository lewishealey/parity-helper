...
const ResultsDisplay = ({ id }: { id: string }) => {
  ...
  const getDescription = (score: number) => {
    ...
    if (score >= MEDIUM_THRESHOLD) return "You have moderate parity";
    if (score >= LOW_THRESHOLD) return "You have low parity";
    return "You have no parity";
  };
  ...
  return (
    <div
      ...
      <SEO
        title={`${results.componentName ? results.componentName : 'Your component '} ${getDescription(results.score)}`}
          ...
          <div className="md:w-1/2 overflow-hidden p-8 space-y-8 flex flex-col justify-between bg-gray-200 rounded-tl-xl rounded-tr-xl lg:rounded-tr-none lg:rounded-tl-lg lg:rounded-bl-lg">
            <div>
              ...
              <div className="space-y-8 px-0 lg:px-8">
                ...
                {specialMessage && <p className="text-lg lg:text-2xl text-gray-800">{specialMessage}</p>}
                <button
